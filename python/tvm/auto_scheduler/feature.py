...
def unpack_feature(byte_arr: bytearray, access_matrix: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if access_matrix: vec_len += ACCESS_MATRIX_VEC_LEN
...
def get_per_store_features_from_file(
    filename: str, max_lines: int, max_n_bufs: Optional[int] = None, access_matrix: bool = True
...
def get_per_store_features_from_measure_pairs(
    ...
    max_n_bufs: Optional[int] = None, access_matrix: bool = True
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def get_per_store_features_from_states(
    states: List[Union[State, StateObject]], task: "SearchTask", max_n_bufs: Optional[int] = None, access_matrix: bool = True
) -> np.ndarray:
    """Get per-store features from measurement input/result pairs
...
def get_per_store_feature_names(max_n_bufs: Optional[int] = None, access_matrix: bool = True) -> List[str]:
    ...
    """
    return _ffi_api.GetPerStoreFeatureNames(max_n_bufs or DEFAULT_MAX_N_BUFS, access_matrix)
