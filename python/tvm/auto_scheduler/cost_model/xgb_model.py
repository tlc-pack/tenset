...
class XGBModelInternal:
    ...
    def make_diff_set(self, base_model, dataset):
        ...
        diff_set = Dataset(self.assembly)
        for task in dataset.tasks():
...
class XGBModel(PythonBasedModel):
    ...
    def __init__(self, few_shot_learning="base_only", verbose_eval=25,
                 num_warmup_sample=100, seed=None, disable_update=False, assembly=True):
        super().__init__()
...
def feature_to_pack_sum_xgbmatrix(xs, assembly=True):
    ...
    feature_names = list(get_per_store_feature_names(assembly=assembly)) + ['max', 'min', 'add', 
...
def pack_sum_xgbmatrix(xs, ys, gids=None, weights=None, assembly=True):
