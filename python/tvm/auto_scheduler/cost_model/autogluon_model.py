...
dmatrix_context = AGDMatrixContext()
...
class AGModelInternal:
    ...
    def _fit_a_model(self, train_set, valid_set=None, valid_train_set=None):
        ...
        hyperparameters = {  # hyperparameters of each model type
                        'XGB': self.xgb_params,  # NOTE: comment this line out if you get errors on Mac OSX
        ...
        bst = TabularPredictor(label='label', eval_metric='root_mean_squared_error').fit(
            dtrain, tuning_data=dtest, time_limit=time_limit,
...
class AGModel(PythonBasedModel):
    """The wrapper of AGModelInternal. So we can use it in end-to-end search."""
    def __init__(self, few_shot_learning="base_only", verbose_eval=25,
        ...
        self.model = AGModelInternal(few_shot_learning=few_shot_learning,
    ...
    def update(self, inputs, results):
        ...
        logger.info("AGModel Training time: %.2f s", time.time() - tic)
    ...
    def update_from_file(self, file_name, n_lines=None):
        ...
        logger.info("AGModel: Loaded %s measurement records from %s", len(inputs), file_name)
...
def pack_sum_xgbmatrix(xs, ys, gids=None, weights=None):
    ...
    ret = pd.DataFrame(np.array(x_flatten), columns=feature_names)
    ret['label'] = y_flatten
    ret = TabularDataset(ret)
    ...
    def callback(env):
        ...
        if (maximize_score and score > best_score) or \
            ...
            if verbose_eval and env.rank == 0:
                logger.debug("AG stopped. Best iteration: %s ", best_msg)       
